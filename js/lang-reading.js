...
async function buildLangStory(id,lvl,situation){
 ...
 if(S.geminiKey){
  try{
   ...
   const obj=await geminiJSON('Eres autor de lecturas de '+langInfo(id).name+' para un adulto hispanohablante nivel '+CEFR_LEVELS[lvl]+'. Escribe un texto corto (100 a 150 palabras) en '+langInfo(id).name+' sobre "'+LANG_SITUATION_LABEL[situation]+'", usando naturalmente parte de este vocabulario: '+vocabTxt+'. Luego arma un glosario con la traducción al español de TODAS las palabras no triviales del texto (sustantivos, verbos, adjetivos — no artículos ni preposiciones comunes). Responde SOLO JSON: {"title":"título corto","text":"el texto completo","words":{"palabra1":"traducción1","palabra2":"traducción2"}}. Las claves de "words" deben estar en minúsculas y sin puntuación, tal como aparecen en el texto.');
   if(obj.text&&obj.words)return{title:stripHTML(obj.title||LANG_SITUATION_LABEL[situation]),text:stripHTML(obj.text),words:obj.words};
  }catch(e){}
 }
 ...
 return{title:LANG_SITUATION_LABEL[situation],text:text,words:words};}
