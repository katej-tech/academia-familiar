...
async function screenLangComic(id,lvl,situation,vocab){
 ...
 try{
  ...
  list.unshift({img,langId:id,situation,ts:Date.now()});
  while(list.length>20)list.pop();
  save();
  renderLangComicResult(img);
 }catch(e){
...
function renderLangComicResult(img){
 render(topbar("screenLangHub()")
  ...
  +'<button class="abtn" style="margin-top:12px" onclick="retryLangComic()">🔄 Generar otra</button>'
...
function screenLangComicGallery(){setTheme("adulto");
 ...
 render(topbar("screenLangHub()")
  ...
  +(list.length?list.map(c=>'<div class="card"><img src="'+c.img+'" style="width:100%;border-radius:12px" alt="Historieta"><p class="mut" style="margin-top:6px">'+langInfo(c.langId).flag+' '+(LANG_SITUATION_LABEL[c.situation]||"")+'</p></div>').join(""):'<p class="mut center">Aún no has generado historietas. Termina una lección y toca "Ver historieta".</p>'));}
